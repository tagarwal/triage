...
/** Representation of a testlogic.log test results file.
 */

public class TestlogicResults extends DatedFile implements Constants {

    public TestlogicResults(VirtualFile root) {
        super(root);
    }
    ...
    private static String computeUrl(UnitTestlogicReport utr, VirtualFile f) {
         ...
         if (!success) {
             ...
             if (utr.getDiffs() > 0) {
                 ...
                 summary = summary + "Build failure - testlogic.log file.";
             }
         }
    ...
    static void readTestResults(VirtualFile f) {
        TestlogicResults tr = new TestlogicResults(f);
        tr.setEndTime(f.lastModified());
        tr.setStartTime(f.lastModified());
        if (true) { //(!tr.getName().startsWith("TESTS-")) {
            // ensure we are not double-counting

            UnitTestlogicReport utr = new UnitTestlogicReport(f);
/*
